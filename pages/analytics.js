  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const pr = await fetch(`/api/products?editToken=${encodeURIComponent(editToken)}`);
        const pj = await pr.json();
        if (!pj?.ok) throw new Error(pj?.error || "Failed to load products");
        // analytics
        const r7 = await fetch(`/api/analytics-summary?editToken=${encodeURIComponent(editToken)}&days=7`);
        const j7 = await r7.json();
        const r30 = await fetch(`/api/analytics-summary?editToken=${encodeURIComponent(editToken)}&days=30`);
